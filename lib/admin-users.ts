 ...
 *
 * Employees are limited admin users (can add/edit properties only). They're
 * persisted to `data/admin-users.json` alongside the other file-based data.
 * Passwords are stored in plaintext to match the existing simple admin scheme
 * — fine for an internal tool, but swap for hashing if this ever goes public.
 */
