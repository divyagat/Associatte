...
// Google Maps (install: npm install @react-google-maps/api)
...
const CITY_COORDS: Record<string, { lat: number; lng: number; zoom: number }> = {
  ...
  kdmc: { lat: 19.2183, lng: 73.0789, zoom: 12 }, // Kalyan-Dombivli
  default: { lat: 19.0760, lng: 72.8777, zoom: 10 }
};
...
export default function CityPage() {
  ...
  const [mapCenter, setMapCenter] = useState(CITY_COORDS[cityName] || CITY_COORDS.default);
  ...
  const filteredProjects = useMemo(() => {
    ...
    else if (sortBy === 'price-desc') projects.sort((a, b) => parsePrice(b.price) - parsePrice(a.price));
  ...
  useEffect(() => {
    const coords = CITY_COORDS[cityName] || CITY_COORDS.default;
    setMapCenter(coords);
  }, [cityName]);
  ...
  // Show loading/error states for map
  if (loadError) {
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 text-gray-600">
                ...
                <span className="text-sm font-medium">{totalProjects} properties found</span>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
            ...
            <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between bg-gray-50">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-gray-700">{totalProjects} properties on map</span>
              </div>
              <button 
                onClick={() => setMapCenter(CITY_COORDS[cityName] || CITY_COORDS.default)}
        ...
        <main className="max-w-7xl mx-auto px-6 py-8">
          {totalProjects === 0 ? (
            <div className="text-center py-20">
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-3">No projects match your filters</h3>
                      ...
                      <div className="p-5">
                        <div className="mb-4">
                          ...
                          <p className="text-xs text-gray-400 mt-0.5 flex items-center gap-1">
                            ...
                            {project.fullLocation?.area}, {project.fullLocation?.city || cityDisplayName}
                          </p>
                        </div>
          ...
          {totalProjects > 6 && viewMode !== 'map' && (
            <div className="mt-12 text-center">
              ...
              <p className="text-xs text-gray-500 mt-3">Showing 1-{Math.min(6, totalProjects)} of {totalProjects} projects</p>
            </div>
          )}
        </main>
      )}
