...
const getAllBuilders = () => {
  ...
  properties.forEach((p: any) => {
    ...
    if (!builderMap.has(slug)) {
      builderMap.set(slug, {
        ...
        projects: [], locations: new Set<string>(), totalProjects: 0,
      });
    }
  ...
  return Array.from(builderMap.values())
    .map(b => ({ ...b, locations: Array.from(b.locations) }))
...
export default async function BuildersPage({ 
  ...
  const filteredBuilders = allBuilders.filter(builder => {
    const matchesSearch = !searchQuery || 
      ...
      builder.locations.some(loc => loc.toLowerCase().includes(searchQuery.toLowerCase()));
    const matchesLocation = !locationFilter || builder.locations.includes(locationFilter);
    return matchesSearch && matchesLocation;
  });

  const totalBuilders = allBuilders.length;
  const totalProjects = allBuilders.reduce((sum, b) => sum + b.totalProjects, 0);
  const locations = Array.from(new Set(allBuilders.flatMap(b => b.locations))).sort();
    ...
    <main className="min-h-screen bg-gray-50">
          ...
          <div className="max-w-3xl">
            ...
            <form className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                <input type="text" name="q" placeholder="Search builder name or locality..." defaultValue={searchQuery}
              ...
              <button type="submit" className="px-6 py-3 bg-[#F8C21C] text-[#8B0000] font-semibold rounded-xl hover:bg-[#d4a017] transition-colors">Search</button>
            </form>
            {(searchQuery || locationFilter) && (
              <div className="flex flex-wrap gap-2 mt-4">
                {searchQuery && <span className="inline-flex items-center gap-1 px-3 py-1 bg-white/10 rounded-full text-sm">Search: "{searchQuery}" <a href="/builders" className="hover:text-[#F8C21C]">×</a></span>}
                {locationFilter && <span className="inline-flex items-center gap-1 px-3 py-1 bg-white/10 rounded-full text-sm">Location: {locationFilter} <a href="/builders" className="hover:text-[#F8C21C]">×</a></span>}
              </div>
            )}
          </div>
        </div>
      ...
      <section className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-wrap justify-center gap-6 text-sm">
          <span className="flex items-center gap-2 text-gray-600"><span className="w-2 h-2 rounded-full bg-[#005E60]"></span><strong className="text-gray-900">{totalBuilders}</strong> Verified Builders</span>
          <span className="flex items-center gap-2 text-gray-600"><span className="w-2 h-2 rounded-full bg-[#F8C21C]"></span><strong className="text-gray-900">{totalProjects}</strong> Total Projects</span>
          <span className="flex items-center gap-2 text-gray-600"><span className="w-2 h-2 rounded-full bg-[#8B0000]"></span><strong className="text-gray-900">{locations.length}</strong> Locations</span>
        </div>
      </section>
      ...
      <section className="bg-[#8B0000] text-white py-12 text-center">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="text-white/90 mb-6 max-w-2xl mx-auto">We're constantly adding new verified developers. Reach out to our team for assistance.</p>
          <a href="tel:+918668695995" className="inline-flex items-center gap-2 px-6 py-3 bg-[#F8C21C] text-[#8B0000] font-semibold rounded-xl hover:bg-[#d4a017] transition-colors">
